...
public interface ICopilotClient
{
    ...
    Task<CopilotConversation?> SendMessageAsync(string conversationId, string message, string? timeZone = null);
    ...
    /// <param name="message">The message text to send.</param>
    /// <param name="timeZone">Optional timezone identifier for locationHint.</param>
    /// <returns>A task that represents the asynchronous operation. The task result contains the stream for SSE events.</returns>
    Task<Stream?> SendStreamingMessageAsync(string conversationId, string message, string? timeZone = null);
}
